...
if(N_clades > 0){
 ...
 if(i == 1){ 
              t_clade[i] ~ gumbel(tmrca + log(lambda_n/omega_p)/lambda_n,1/lambda_n) T[tmrca,times[1]];
  }
...
if(N_cd > 0){
  ...
  for (i in 1:N_cd){
    ...
    t_cd[i,1] ~ gumbel(tmrca + log(lambda_n/omega_p)/lambda_n,1/lambda_n) T[tmrca,times[1]]; 
